...
export default class TypeAhead extends Component<TypeAheadProps,TypeAheadState> {
    ...
    mapOptions(options: Array<any>) {
        ...
        options.forEach(option => {
            tempOptions.push({label:option[this.props.labelKey], value: option[this.props.valueKey]});
        });
    ...
    mapValues(values: any) {
        ...
        values.forEach((value:any) => {
            tempValues.push({label:value[this.props.labelKey], value: value[this.props.valueKey]});
        });
    ...
    emitChange(selected: any) {
        ...
        if(!selected) { // React select gives null on empty but we need empty array
            this.props.onChange([]);
            this.setState({valuesToPass:selected});
            return;
        }
